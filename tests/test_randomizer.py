...
def test_five_drop_guarantee(default_star_roller):
    for _ in range(100):
        rolls = [default_star_roller.roll_star() for __ in range(90)]
        assert len(rolls) == 90
        assert Rarity.FIVE in rolls, "За 90 роллов должен гарантированно упасть хотя бы 1 пятый дроп"


def test_four_drop_guarantee(default_star_roller):
    for _ in range(100):
        rolls = [default_star_roller.roll_star() for __ in range(10)]
        assert len(rolls) == 10
